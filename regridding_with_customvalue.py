...
# Ejemplo de uso:
# python regridding_with_customvalue.py './TEST/*.nc4' '/home/msuarez/Documents/Regridding/geo_em.d03.nc' './TEST/output' '2021-03-16 00:15' '2021-03-16 02:15' '50'
...
parser.add_argument("inicio", type=str, help='Fecha de inicio de la precipitacion. Ej: 2021-03-16 00:15')
parser.add_argument("fin", type=str, help='Fecha de fin de la precipitacion. Ej: 2021-03-16 02:15')
parser.add_argument("rainrate", type=float, help='Tasa de precipitación a asignar (en mm/h). Ej: 50')
...
init_date = args.inicio
end_date = args.fin
custom_rain_rate = args.rainrate

custom_rain_rate = float(custom_rain_rate)
...
# for i in range(13,27):
    ...
    # Le asigno un valor datetime a la mitad de la lectura del satelite
    ...
    #############################################################################################################
    #############################################################################################################
    ################################ SETEO TODOS LOS VALORES DE PRECIP A 0 ######################################
    if init_date <= datetime_object <= end_date: 
        temp_array_filtered[:,:] = custom_rain_rate
    else:
        temp_array_filtered[:,:] = 0
    #############################################################################################################
    #############################################################################################################


    rainrate = ncout.createVariable('precip_rate','float32',('Time','south_north','west_east'))
    rainrate.setncattr('units','mm/s'); rainrate[:] = temp_array_filtered/3600;
    ...
    if plot_imerg:
        ...
        draw_screen_poly( lats_red, lons_red, m, 'red' )
